...
def plot_eigensystem(eigvals: np.ndarray, eigvecs: np.ndarray, T: int, N: int,
	...
	plt.plot(lam_grid, mp_density, "-", color="#F58518", linewidth=2.2, label="RMT (MP) density")
	...
	for k, ax in enumerate(axes.flat):
		if k < len(order_indices):
			...
			ax.plot(x_grid, normal_pdf, "-", color="#EECA3B", linewidth=2.2, label="Standard normal")
			ev = eigvals[col_idx]
